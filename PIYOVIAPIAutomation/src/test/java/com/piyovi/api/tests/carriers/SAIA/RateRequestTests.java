...
public class RateRequestTests extends BasePage {
    ...
    public void initialize() throws IOException {
        RestAssured.baseURI = propertyReader.getApplicationProperty("baseURI") + SAIAConstants.RATE_REQUEST_URL;
        this.shipDate = dttmUtil.getDateTimeInFormat(this.shipDtTmFormat);
    }

    @Test
    public void testRateRequestOK() {
        logger = extent.createTest("SAIA API - Test Basic Rate Request", "Verify Basic Rate Request");

        var fileHelper = new FileHelper();
        var payload = fileHelper.getFile(carriersPayLoadPath+SAIAConstants.RATE_REQUEST_PAYLOAD);
        ...
        for(int index=0; index < array.length(); index++) {  
            ...
            assertEquals(carrierName,"Saia");
            
            if(serviceName.toUpperCase().contains("STD")) {
            	logger.info("Actual Service Name: " + serviceName);
                logger.info("Expected Service Name: STD");
            	assertEquals(serviceName,"STD");
            	...
            	logger.info("Actual Service Name: " + serviceName);
                logger.info("Expected Service Name: GSS5");
            	assertEquals(serviceName,"GSS5");
            	...
            	logger.info("Actual Service Name: " + serviceName);
                logger.info("Expected Service Name: GSS12");
            	assertEquals(serviceName,"GSS12");
            	
            	logger.info("Actual Service Description: " + description);
                logger.info("Expected Service Description: Guaranteed by 12 Noon");
            	assertEquals(description,"Guaranteed by 12 Noon");
            }
        }  
    }
}
