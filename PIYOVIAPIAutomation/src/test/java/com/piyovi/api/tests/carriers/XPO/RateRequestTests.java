...
public class RateRequestTests extends BasePage {
    ...
    public void initialize() throws IOException {
        RestAssured.baseURI = propertyReader.getApplicationProperty("baseURI") + XPOConstants.RATE_REQUEST_URL;
        this.shipDate = dttmUtil.getDateTimeInFormat(this.shipDtTmFormat);
    }

    @Test
    public void testRateRequestOK() {
        logger = extent.createTest("XPO API - Test Basic Rate Request", "Verify Basic Rate Request");

        var fileHelper = new FileHelper();
        var payload = fileHelper.getFile(carriersPayLoadPath+XPOConstants.RATE_REQUEST_PAYLOAD);
        ...
        for(int index=0; index < array.length(); index++) {  
            ...
            assertEquals(carrierName,"XPOLogistics");
            
            if(serviceName.toUpperCase().contains("STD")) {
            	logger.info("Actual Service Name: " + serviceName);
                logger.info("Expected Service Name: XPOSTD");
            	assertEquals(serviceName,"XPOSTD");
            	...
            	logger.info("Actual Service Name: " + serviceName);
                logger.info("Expected Service Name: XPOGSS5");
            	assertEquals(serviceName,"XPOGSS5");
