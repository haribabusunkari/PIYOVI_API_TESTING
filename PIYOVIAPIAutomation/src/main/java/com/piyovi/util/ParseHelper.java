...
public class ParseHelper {
    ...
    public Object parsAccordingToType(Object value, String... desaireType) {
        String type;
        if (desaireType.length == 0)
            type = value.getClass().getSimpleName();
        else
            type = desaireType[0];
        switch (type) {
            case "Integer" -> {
                return parsStringToInt(String.valueOf(value));
            }
            case "BigInteger" -> {
                return parsStringToBigint((String) value);
            }
            case "Boolean" -> {
                return parseStringToBoolean(String.valueOf(value));
            }
            case "Float" -> {
                return Math.round((parsStringToFloat(String.valueOf(value)) * 100.00)) / 100.00;
            }
            case "Double" -> {
                return Math.round((parsStringToDouble(String.valueOf(value)) * 100.00)) / 100.00;
            }
            case "String" -> {
                return String.valueOf(value);
            }
            default ->
                    throw new IllegalArgumentException(String.format("Undefined type %s of %s ", Arrays.toString(desaireType), value));
        }
    }
}
