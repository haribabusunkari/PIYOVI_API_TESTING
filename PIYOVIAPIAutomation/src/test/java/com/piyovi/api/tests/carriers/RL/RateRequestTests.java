...
public class RateRequestTests extends BasePage {
    ...
    public void initialize() throws IOException {
        RestAssured.baseURI = propertyReader.getApplicationProperty("baseURI") + RLConstants.RATE_REQUEST_URL;
    }

    @Test
    public void testRateRequestOK() {
        logger = extent.createTest("RL API - Test Basic Rate Request", "Verify Basic Rate Request");

        var fileHelper = new FileHelper();
        var payload = fileHelper.getFile(carriersPayLoadPath+RLConstants.RATE_REQUEST_PAYLOAD);
